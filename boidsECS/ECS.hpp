...
public:
	...
	void foreach(Fn&& fn)
	{
		foreach_inner<Fn, ChunkSize, std::tuple<std::array<size_t, ChunkSize>, std::array<std::remove_cvref_t<Ts>, ChunkSize>...>, typename gmeta::fntraits_t<Fn>::Args_t>{}(std::forward<Fn>(fn), _arrays, _size);
	}
...
private:
	...
	template<typename Fn, size_t _ChunkSize, typename ArraysT, typename _Ts>
	...
	size_t _size;
	std::tuple<std::array<size_t, ChunkSize>, std::array<std::remove_cvref_t<Ts>, ChunkSize>...> _arrays;
	std::unordered_map<Id, size_t> id_to_index_map;
};
